...
const plans = [
  {
    name: "Básico", price: "$2500", label: "Tu primer paso online", image: cafe,
    description: "Una página simple para presentar tu negocio y empezar a conectar con tus clientes.",
  ...
  {
    name: "Avanzado", price: "$4000", label: "Más espacio para crecer", image: barberia,
  ...
  {
    name: "Completo", price: "$7000", label: "Tu negocio, conectado", image: ferreteria,
    ...
    <main className="budget-page">
      <section className="budget-hero" aria-labelledby="budget-title">
        <div className="budget-hero-content">
          <span className="budget-eyebrow">DISEÑO WEB · JL MARKETING</span>
          ...
          <h1 id="budget-title">Tu próxima web.<br /><span>A tu medida.</span></h1>
          <p>Pedí tu presupuesto sin compromiso y descubrí la página que podría tener tu negocio.</p>
          <a className="budget-hero-link" href="#planes">Encontrá tu plan <span aria-hidden="true">↗</span></a>
        </div>
      ...
      <section className="budget-plans" id="planes" aria-labelledby="plans-title">
        <header className="budget-plans-heading">
          <div><span className="budget-eyebrow">UNA WEB PARA CADA ETAPA</span><h2 id="plans-title">Elegí cómo empezar.</h2></div>
          <p>Tres puntos de partida. Un proyecto a tu medida.<br />El presupuesto final se ajusta a tu negocio.</p>
        </header>
        ...
        <div className="budget-grid">
          {plans.map((plan, index) => (
            <article id={`plan-${index}`} key={plan.name} aria-labelledby={`plan-title-${index}`} className={`budget-card ${index === 1 ? "budget-card-featured" : ""} ${selected === index ? "is-selected" : ""}`}>
              <div className="budget-card-top"><span>{plan.label}</span><span className="budget-plan-number">0{index + 1}</span></div>
              <div className="budget-card-title"><h3 id={`plan-title-${index}`}>{plan.name}</h3><div><span>Desde</span><strong>{plan.price}</strong></div></div>
              <p className="budget-description">{plan.description}</p>
        ...
        <p className="budget-footnote">Cada negocio es distinto. Estos planes son el comienzo de tu proyecto.</p>
      </section>
    </main>
  );
}
